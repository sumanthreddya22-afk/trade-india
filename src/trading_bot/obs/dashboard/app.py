...
def _filter_equity_range(points: list[Any], range_key: str) -> list[Any]:
    ...
    Leading zero-equity points (days before trading started — broker
    snapshots that recorded $0 before the account was funded) are dropped
    so the chart isn't squashed by a flat $0 baseline.
    """
    if not points:
        return points
    # Strip leading zeros — these are pre-trading days, not a real equity reading.
    first_real = next((i for i, p in enumerate(points) if float(p.equity) > 0), None)
    if first_real is None:
        return []
    points = points[first_real:]
    if range_key == "all":
        return points
...
def create_app() -> FastAPI:
    ...
    _DEBATE_TABLES: set[str] = {
        "scout_debate_runs", "scout_debate_runs_crypto", "scout_debate_runs_options",
        "entry_debate_runs", "entry_debate_runs_crypto",
        "hold_debate_runs", "hold_debate_runs_crypto",
        "wheel_debate_runs_options", "unblock_debate_runs",
    }
    _SCOUT_TABLES: set[str] = {
        "scout_debate_runs", "scout_debate_runs_crypto", "scout_debate_runs_options",
    }
    _UNDERLYING_TABLES: set[str] = {"wheel_debate_runs_options", "scout_debate_runs_options"}

    @app.get("/api/agent/debates")
    def api_agent_debates(table: str, limit: int = 2) -> Any:
        """Return the latest N rows from a debate table for the transcript panel."""
        if table not in _DEBATE_TABLES:
            raise HTTPException(status_code=400, detail=f"Unknown table: {table!r}")
        import sqlite3 as _sq3
        subject_col = "underlying" if table in _UNDERLYING_TABLES else "symbol"
        if table in _SCOUT_TABLES:
            text_cols = ["skeptic_text", "analyst_text"]
            col_labels = ["Skeptic", "Analyst"]
        else:
            text_cols = ["aggressive_text", "conservative_text", "neutral_text"]
            col_labels = ["Aggressive", "Conservative", "Neutral"]
        cols_csv = ", ".join(text_cols)
        sql = (
            f"SELECT {subject_col}, run_at, verdict, confidence, judge_reason, {cols_csv} "
            f"FROM {table} ORDER BY run_at DESC LIMIT ?"
        )
        try:
            with _sq3.connect(f"file:{STATE_DB_PATH}?mode=ro", uri=True, timeout=5.0) as conn:
                rows_raw = conn.execute(sql, (max(1, min(limit, 10)),)).fetchall()
        except Exception as exc:
            logger.exception("api_agent_debates: query failed for %s", table)
        ...
        for r in rows_raw:
            ...
            for i, label in enumerate(col_labels):
                row[label.lower()] = r[5 + i] or ""
            rows.append(row)
        return JSONResponse({"table": table, "col_labels": col_labels, "rows": rows})
...
def _formatters() -> dict[str, Any]:
    ...
    def short_dt(v: Any) -> str:
        """Locale-portable short date/time, rendered in ET so it matches
        every other timestamp the operator sees on this dashboard."""
        try:
            from zoneinfo import ZoneInfo
