...
def build_action_items(
    *, positions: list, orders: list, state_db: str = "data/state.db",
    ...
    # Crypto dust positions (notional < $10) cannot be stop-protected on
    # Alpaca — the order would fail with error 40310000 "cost basis must be
    # >= minimal amount of order 10". Exclude them from the danger banner;
    # position_protection.py records them as outcome="dust_unprotectable"
    # and they show up in the open-positions email summary instead.
    def _is_crypto_dust(p) -> bool:
        ac = (getattr(p, "asset_class", "") or "").lower()
        if "crypto" not in ac:
            return False
        try:
    ...
    for p in positions or []:
        if _canon(p.symbol) in stops_by_symbol:
            continue
        if _is_crypto_dust(p):
            dust.append(p.symbol)
        else:
    ...
    if dust:
        items.append(ActionItem(
            severity="info",
            headline=f"{len(dust)} crypto dust position{'s' if len(dust) > 1 else ''} (un-stoppable)",
            detail=(
                f"{', '.join(dust[:3])}"
                f"{' …' if len(dust) > 3 else ''} — "
                "notional below Alpaca's $10 crypto minimum; can't place a stop or "
                "flatten via order. Resolve from the Alpaca UI when convenient."
            ),
            target="#stop_coverage",
        ))
