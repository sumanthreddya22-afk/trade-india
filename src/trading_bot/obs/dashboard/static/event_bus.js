...
(function () {
  ...
  const STATUS_LABEL = { live: "live", reconn: "reconn", down: "down", off: "off" };
  const status = (s) => {
    document.body.dataset.streamStatus = s;
    // Mirror the status into every .stream-pill .label so the operator
    // sees text, not just a coloured dot.
    document.querySelectorAll(".stream-pill .label").forEach((el) => {
