...
_LLM_OUTPUT_KINDS = {"scout", "entry", "wheel"}
# "hold" intentionally excluded: hold debates are reactive (fire on position
# triggers), so count=0 simply means no positions needed attention — healthy.
...
def _stage_health(
    ...
    *,
    ...
    Rules:
      - Risk gate stages (kind="risk", no role_name): count = circuit trips.
        Zero trips = all-clear, so always return "ok".
      ...
      - LLM stages (scout/entry/hold/wheel) downgrade to warn when the host
        role ran but produced ZERO debates today — surfaces the common failure
        mode where the cron fires but no debates land.
      - Lesson stages are nightly — count=0 is expected for new accounts (no
        completed trades yet). Healthy if role ran recently without error.
      - Stages with a role_name look at role_runs:
          last status fail AND no recent success → fail
          last run >36h ago → warn
    ...
    # Risk gate: zero trips = healthy (the gate is quiet, not broken).
    if stage.kind == "risk" and not stage.role_name:
        return "ok"
    ...
    if age_hours > stage.stale_hours:
        return "warn"

    # Lesson stages run once per night; skip the runs_today and LLM-output
    # count checks — count=0 simply means no closed trades to learn from yet.
    if stage.kind == "lesson":
        return "ok"

    # Market-hours-only stages (stale_hours > 36) legitimately don't fire on
    # weekends or holidays. If the role ran within its tolerance window, skip
    # the intra-day activity checks — they would always fire on weekends.
    if stage.stale_hours > 36:
        return "ok"
