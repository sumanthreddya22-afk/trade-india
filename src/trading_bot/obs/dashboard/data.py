...
class PositionRow:
    ...
    # v4 Phase 0: provenance — bot|external|manual|unknown. Defaults to
    # "unknown" so older callers that don't populate it still render.
...
class LessonsBlock:
    ...
    *learned* from recent closed trades — not just what it did."""
    ...
    # Diagnostics so the empty-state can be honest about *why* it's empty:
    # the reflector joins closed_trades.entry_order_id → decisions.entry_order_id,
    # and when crypto/wheel/legacy entries didn't stamp that key the join
    # produces zero candidates — the panel should say so rather than "first
    # batch lands after the first closed trade".
    closed_trades_total: int = 0       # rows in closed_trades.db
    closed_trades_joinable: int = 0    # rows that match a decisions row
    ...
    *,
    ...
    for p in positions:
        try:
            ...
            # v4 Phase 0: classify provenance. The Alpaca Position object
            # doesn't expose client_order_id directly; until the Phase 1
            # ledger ships, everything classifies as "external" unless the
            # legacy bot-prefix matches via downstream lookup. For now we
            # mark all positions "unknown" until the ledger join lands —
            # the column exists so the dashboard can render and the test
            # surface is in place.
            from trading_bot.position_classifier import (
                BrokerPosition as _BP,
                classify as _classify_pos,
            )
            classification = _classify_pos(_BP(
                symbol=p.symbol,
                asset_class=getattr(p, "asset_class", "") or "",
...
_KNOWN_SCHEDULED_JOBS: list[tuple[str, str, str]] = [
    ...
    ("nightly_review", "Nightly self-review (Bucket G read-only digest)", "0 17 * * *"),
    ("event_bus_retention", "Event-bus retention (DELETE older than 7d)", "15 3 * * *"),
    ("debate_outcome_analyzer", "Debate outcome analyzer (3 pipelines, nightly)", "30 18 * * *"),
    # ─ Daemon (24/7 micro-cadence intel + execution) ─────────────────────
    ("crypto_intel_ingestor", "Crypto intel ingest (every 30 min, 24/7)", "*/30 * * * *"),
    ("stock_intel_ingestor", "Stock intel ingest (every 30 min RTH)", "*/30 9-16 * * 1-5"),
    ("crypto_streamer", "Crypto streamer (every minute, 24/7 express lane)", "* * * * *"),
    ("position_monitor", "Position monitor (24/7 hold-debate trigger)", "0 * * * *"),
    # ─ Daemon (options/wheel — Phase 3) ──────────────────────────────────
    ("options_scanner", "Options scanner (earnings + skew, scout debate)", "30 9 * * 1-5"),
    ("wheel_entry_debate", "Wheel entry debate (legacy + new fusion)", "20 10 * * 1-5"),
    ...
    ("decision_reflect", "Lab — Decision Reflector (post-mortems)", "30 3 * * *"),
    ("threshold_tune", "Lab — Threshold tuner (nightly knobs)", "0 4 * * *"),
    ("intel_ingest_market", "Lab — Intel ingest (market hours, every 30 min)", "0,30 9-15 * * 1-5"),
    ("intel_ingest_offhours", "Lab — Intel ingest (off-hours, hourly)", "0 0-8,16-23 * * *"),
]
...
def build_snapshot(
    *,
    ...
    kpi = KpiBlock(
        ...
        # Carry over the fallback-gate state — the rebuild above only folds
        # in equity-curve-derived fields, but dropping these reset the
        # Trading Gate card to "state unknown".
        fallback_active=kpi.fallback_active,
        fallback_set_at=kpi.fallback_set_at,
        fallback_set_by=kpi.fallback_set_by,
        fallback_reason=kpi.fallback_reason,
    )
...
def _build_decision_activity(
    ...
    # Decisions are stored with a space separator (`2026-05-11 00:32:15.000000`),
    # not ISO-`T`. Lexical compare with `T` truncates the window to ~today.
...
def _build_decision_lessons(
    ...
    # Diagnose join health up front so the empty-state can explain *why*
    # the panel is empty. closed_trades lives in its own SQLite file.
    closed_total = 0
    closed_joinable = 0
    closed_db = _os.environ.get("TRADING_BOT_CLOSED_TRADES_DB", "data/closed_trades.db")
    try:
        if _os.path.exists(closed_db):
            with sqlite3.connect(closed_db) as ccon:
                row = ccon.execute(
                    "SELECT entry_order_id FROM closed_trades"
                ).fetchall()
            entry_ids = [r[0] for r in row if r[0]]
            closed_total = len(row)
            if entry_ids:
                # Count how many close-out entry_order_ids are present in
                # decisions — that's the upper bound on what reflector
                # can post-mortem.
                qmarks = ",".join("?" * len(entry_ids))
                with sqlite3.connect(state_db_path) as scon:
                    closed_joinable = (scon.execute(
                        f"SELECT COUNT(*) FROM decisions WHERE entry_order_id IN ({qmarks})",
                        entry_ids,
                    ).fetchone() or [0])[0]
    except Exception:
        # Diagnostic is best-effort; don't fail the panel on it.
        pass
    ...
    try:
        with sqlite3.connect(state_db_path) as conn:
            try:
                ...
                return LessonsBlock(
                    rows=[], total=0, last_lesson_at=None,
                    closed_trades_total=closed_total,
                    closed_trades_joinable=closed_joinable,
                )
            try:
                total = (conn.execute(
    ...
    if not rows:
        return LessonsBlock(
            rows=[], total=int(total or 0), last_lesson_at=None,
            closed_trades_total=closed_total,
            closed_trades_joinable=closed_joinable,
        )
