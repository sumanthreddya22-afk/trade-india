...
LAB_LABEL="com.bharath.trading.lab"
...
cp "$REPO_ROOT/ops/launchd/${LAB_LABEL}.plist" "$LAUNCHD_DIR/"
...
launchctl unload "$LAUNCHD_DIR/${LAB_LABEL}.plist" 2>/dev/null || true
launchctl load -w "$LAUNCHD_DIR/${DAEMON_LABEL}.plist"
launchctl load -w "$LAUNCHD_DIR/${SUPERVISOR_LABEL}.plist"
launchctl load -w "$LAUNCHD_DIR/${LAB_LABEL}.plist"

echo "Installed and loaded:"
launchctl list | grep -E "${DAEMON_LABEL}|${SUPERVISOR_LABEL}|${LAB_LABEL}" || true
echo
