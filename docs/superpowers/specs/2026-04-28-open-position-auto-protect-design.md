# Open Position Auto-Protect — Design
...
- A position is "unprotected" when no live stop / stop-limit order references it.
- Once a stop is placed (or position is closed), subsequent sweeps see no unprotected positions and take no further action — so the cadence does not cause repeated emails.

## Decision rule per unprotected position

For each unprotected symbol, fetch 60d daily bars, compute indicators (`rsi_14`, `macd`, `macd_signal`, `ema_20`, `last_close`, `return_5d`) using the existing pipeline (`MarketDataClient.get_daily_bars` + `compute_indicators` from `market_data.py`).

Compute the **strategy-aligned protective stop**:

```
protective_stop = max(ema_20, last_close * (1 - unprotected_stop_pct))
```
...
Combined rule, per unprotected position:
...
risk:
  # ... existing keys ...
  unprotected_stop_pct: 0.05   # 5% default; matches MomentumStrategy default
```
...
- If no unprotected positions found: **no email** (current behavior).
...
This replaces the existing red "unprotected positions" warning intro with a summary line at the top. The visual styling (colors, KPI grid, pills) reuses the helpers already in [`reports.py`](../../src/trading_bot/reports.py).
...
New file: `src/trading_bot/position_protection.py` containing:

- `@dataclass ProtectionAction` — the per-symbol record described above.
- `def evaluate_and_act(client: AlpacaClient, market_data: MarketDataClient, unprotected_positions: list[UnprotectedPosition], *, stop_pct: Decimal, now_in_market_hours: bool) -> list[ProtectionAction]` — pure orchestrator; takes injected clients so tests don't hit Alpaca.
...
2. Filter to unprotected (existing logic, factored into a helper).
3. If list is empty → return.
4. Build inputs, call `evaluate_and_act`.
5. Build email via `build_open_positions_email_html`, send.

The logging surface (`[verify-stops] positions=… stops=… unprotected=…`) keeps emitting per-position outcomes so the daemon log stays useful.
...
- **No regime-aware stop logic.** A single `unprotected_stop_pct` knob; revisit later if needed.
- **No partial position handling.** Whole position is protected or flattened.
- **No re-entry logic.** Closing an unprotected position does not queue a re-entry; that's the orchestrator's job on its normal cadence.
...
4. **Off-hours unprotected stock that needs flattening but gaps further down before RTH.** If a stock is in "broken" territory off-hours and we defer the flatten, it can drop further before the next session's open. Mitigation: the more common case (stop leg detached on a still-healthy position) gets the GTC stop placed immediately and is fine. Only positions already past the EMA-20 + 5% floor are deferred — and those are by definition already in trouble; another few hours of off-hours exposure is a small marginal risk relative to the wider set of overnight-gap risks the bot already accepts.
